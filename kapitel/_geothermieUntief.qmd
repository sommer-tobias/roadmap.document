...
### `r params$genutzt` (`r roundTo(geothermieUntief$ausbaupfad$genutzt$heizwärme_gwh,10)` GWh)
...
Volllaststunden einer jährlichen Entzugsenergie von `r roundTo(geothermieUntief$ausbaupfad$genutzt$umweltwärme_gwh, 1)`
GWh. Bei einer mittleren Jahresarbeitszahl von `r roundTo(parameters$geothermieUntief$JAZ, 0.1)`
ergibt sich so eine Heizwärme von etwa `r roundTo(geothermieUntief$ausbaupfad$genutzt$heizwärme_gwh ,10)` GWh. 

### Theoretisches Potenzial (`r roundTo(geothermieUntief$ausbaupfad$theoretisch$heizwärme_gwh, 100)` GWh)
...
kWh/m^2^ und die für Erdsonden erlaubte Bauzonenfläche angenommen. Zum Heizen werden dabei dem Untergrund `r roundTo(geothermieUntief$ausbaupfad$theoretisch$umweltwärme_gwh, 100)`
GWh entzogen und es resultieren `r roundTo(geothermieUntief$ausbaupfad$theoretisch$heizwärme_gwh, 100)`
GWh Heizwärme (Jahresarbeitszahl von `r roundTo(parameters$geothermieUntief$JAZ, 0.1)`). 


### Realistisches Potenzial (`r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärme_gwh,100)` GWh)
Das nachhaltig nutzbare, also das realistische Heizwärmepotenzial von Erdwärmesonden, liegt in der Schweiz ohne Regeneration bei etwa `r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärmeEP2050plusOhneRegeneration_gwh, 1)`
GWh [@EP2050T, Tab. 14] und mit Regeneration bei etwa `r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärmeEP2050plusMitRegeneration_gwh, 1)`
GWh, wobei Zonen mit nutzbarem Grundwasservorkommen bereits ausgeschlossen wurden. Skaliert auf den Kanton Luzern ergibt dies `r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärmeEP2050plusSkaliertAufKLUohneRegeneration_gwh, 100)`
GWh respektive `r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärmeEP2050plusSkaliertAufKLUmitRegeneration_gwh, 100)`
...
kWh/m^2^ sogar ohne Regeneration möglich sind. Als Arbeitswert wird trotzdem ein realistisches Potenzial von `r roundTo(geothermieUntief$ausbaupfad$realistisch$heizwärme_gwh,100)` GWh angenommen und Regeneration in dicht bebauten Gebieten vorausgesetzt. Für eine konkretere Abschätzung des realistischen Potenzials ist eine Analyse des Luzerner Gebäudeparks inklusive existierenden Heizungsträgern, lokalen Wärmeentzugsdichten und verfügbaren Flächen nötig. 


### `r params$ziel` (`r roundTo(geothermieUntief$ausbaupfad$ziel$heizwärme_gwh, 100)` GWh)
Entwickeln sich die realisierten Erdsondenmeter bis 2050 ähnlich wie von 2021 auf 2022, dann werden im Jahr 2050 `r roundTo(geothermieUntief$ausbaupfad$ziel$heizwärme_gwh, 100)`
