...
### `r params$genutzt` (`r roundTo(luft$ausbaupfad$genutzt$heizwärme_gwh,10)` GWh)
...
Im Jahr 2020 betrug die totale Heizleistung von Luft-Wasser-Wärmepumpen etwa `r roundTo(parameters$luft$totaleInstallierteHeizleistungKLU_mw, 10)`
 MW. Mit `r roundTo(parameters$luft$volllasstunden, 10)`
  Vollaststunden ergibt dies eine Heizwärme von `r roundTo(luft$ausbaupfad$genutzt$heizwärme_gwh, 10)`
...
### Realistisches Potenzial (`r roundTo(luft$ausbaupfad$realistisch$heizwärme_gwh,10)` GWh)
Das realistische Potenzial orientiert sich am realistischen Potenzial der EP2050+. Darin werden Schallgrenzwerte und die begrenzte Leistung von Luft-Wasser-Wärmepumpen berücksichtigt. Skaliert auf den Kanton Luzern ergibt dies `r roundTo(luft$ausbaupfad$realistisch$heizwärme_gwh, 10)`
...
### `r params$ziel` (`r roundTo(luft$ausbaupfad$ziel$heizwärme_gwh, 10)` GWh)

Gleich wie beim realistische Potenzial wird das Ausbauziel von den EP2050+ abgeleitet. Das Ausbauziel beträgt somit `r roundTo(luft$ausbaupfad$ziel$heizwärme_gwh, 10)` GWh.
{{< pagebreak >}}
