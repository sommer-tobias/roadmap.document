...
### `r params$genutzt` (`r roundTo(solarenergieThermisch$ausbaupfad$genutzt$heizwärme_gwh,1)` GWh)
Im Jahr 2020 wurden etwa `r roundTo(solarenergieThermisch$ausbaupfad$genutzt$heizwärme_gwh, 1)`
...
### Realistisches Potenzial (`r roundTo(solarenergieThermisch$ausbaupfad$realistisch$heizwärme_gwh,10)` GWh)
Das Potenzial für Solarwärme auf Dächern in der Schweiz beträgt `r roundTo(solarenergieThermisch$ausbaupfad$ziel$heizwärmeCH_twh ,1)` TWh und im Kanton Luzern `r roundTo(solarenergieThermisch$ausbaupfad$realistisch$heizwärme_gwh,10)` GWh[^flächenkonfliktPV] [@sonnendach]. 
