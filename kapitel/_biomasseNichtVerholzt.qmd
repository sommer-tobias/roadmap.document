...
Im Kanton Luzern gibt es insgesamt 31 Biogas produzierende Anlagen: `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$anzahlARAs, 1)` Abwasserreinigungsanlagen [@biogasHolinger2022], drei industrielle Anlagen[^_biomassenichtverholzt-1] und 13 landwirtschaftliche Co-Vergärungsanlagen. Alle Anlagen produzieren mittels Blockheizkraftwerk Strom und Wärme, nur zwei speisen Biogas zusätzlich in das Gasnetz ein[^_biomassenichtverholzt-2]. Die Stromproduktion aller Biogasanlagen im Jahr 2022 betrug `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$pronovoStromproduktion2022_gwh, 1)` GWh[^_biomassenichtverholzt-3]. Die aus dieser Stromproduktion abgeschätzte genutzte Wärme beträgt `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$pronovoGenutzteWärme2022_gwh, 1)` GWh[^_biomassenichtverholzt-4]. Insgesamt resultieren `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$pronovoStromWärme2022_gwh, 1)` GWh Strom und Wärme. Die direkt ins Gasnetz eingespeiste Energiemenge beträgt `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$GasEingespeisst2022_gwh, 1)` GWh[^_biomassenichtverholzt-5]. In der Summe ist die aktuelle Nutzung somit `r roundTo(biomasseNichtVerholzt$ausbaupfad$genutzt$pronovoGenutzt2022_gwh, 1)` GWh[^_biomassenichtverholzt-6].

[^_biomassenichtverholzt-1]: Emmi Schweiz AG Dagmersellen, Kompogasanlage Wauwil, SwissFarmerPower Inwil

[^_biomassenichtverholzt-2]: ARA Buholz und SwissFamerPowerInwil

[^_biomassenichtverholzt-3]: Pronovo Daten, d.h. ohne Eigenbedarf

[^_biomassenichtverholzt-4]: Annahmen: Strom und Wärmeproduktion stehen im Verhältnis `r roundTo(parameters$wärmekraftkopplung$grobschätzungEndenergie$anteilStrom*100, 1)`:`r roundTo((1 - parameters$wärmekraftkopplung$grobschätzungEndenergie$anteilStrom)*100, 1)` und `r roundTo(parameters$wärmekraftkopplung$grobschätzungEndenergie$anteilGenutzerWärmeAnProduzierterWärme*100, 1)` % der produzierten Wärme werden genutzt
